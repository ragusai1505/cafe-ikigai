      ...
      <div className="p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-coffee-400 text-sm mt-1">Manage your menu items and categories</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link to="/admin/add"
              className="flex items-center gap-2 bg-coffee-600 hover:bg-coffee-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-all shadow-sm hover:shadow-md">
          ...
          {[
            ...
            <div key={label} className="bg-white rounded-2xl p-5 shadow-sm border border-coffee-50">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${
                color === 'coffee' ? 'bg-coffee-100' :
                color === 'green' ? 'bg-green-100' :
                color === 'amber' ? 'bg-amber-100' : 'bg-blue-100'
              }`}>
                <Icon size={20} className={
                  color === 'coffee' ? 'text-coffee-600' :
                  ...
                  <tr key={item.id} className="hover:bg-coffee-50/30 transition-colors group">
                    ...
                    <td className="px-4 py-4 text-center hidden sm:table-cell">
                      <button onClick={() => handleToggleAvail(item)}
                        className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full font-medium transition-all ${
                    ...
                    <td className="px-4 py-4 text-center hidden sm:table-cell">
                      <button onClick={() => handleToggleFeatured(item)}
                        className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full font-medium transition-all ${
                          item.featured ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-coffee-100 text-coffee-500 hover:bg-coffee-200'
                        }`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-coffee-950/60 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl p-7 max-w-sm w-full shadow-2xl animate-scale-in">
            ...
            <p className="text-coffee-400 text-sm text-center mb-6">
              "{menuItems.find(i => i.id === deleteConfirm)?.name}" will be permanently removed.
            </p>
