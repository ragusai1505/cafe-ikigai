  ...
  if (!item) return (
    <AdminLayout>
      <div className="p-8 text-center">
        <p className="text-brand-400">Item not found.</p>
        <button onClick={()=>navigate('/admin')} className="mt-4 text-brand-600 underline text-sm">Back</button>
      </div>
    </AdminLayout>
  )
    ...
    <AdminLayout>
      <div className="p-8 max-w-3xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="font-display text-3xl text-brand-900">Edit Item</h1>
            <p className="text-brand-400 text-sm mt-0.5">{item.name} — saves to Firebase instantly ⚡</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-blush-50">
            <h2 className="font-display text-lg text-brand-900 mb-4">Item Image</h2>
            <div className="flex bg-blush-50 rounded-lg p-1 mb-4 w-fit">
              {['url','file'].map(mode=>(
                <button key={mode} type="button" onClick={()=>setUploadMode(mode)}
                  className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all capitalize ${uploadMode===mode?'bg-white text-brand-800 shadow-sm':'text-blush-500'}`}>
            ...
            {uploadMode==='url' ? (
              ...
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-blush-200 rounded-xl p-8 cursor-pointer hover:border-brand-400 transition-all">
                <Upload size={28} className="text-blush-300 mb-2"/>
                <span className="text-blush-500 text-sm">Click to upload</span>
                <input type="file" accept="image/*" className="hidden" onChange={handleFileUpload}/>
              </label>
            )}
            {imagePreview && (
              <div className="mt-4 relative w-fit">
                <img src={imagePreview} alt="Preview" className="w-40 h-40 object-cover rounded-xl border border-blush-200" onError={()=>setImagePreview('')}/>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-blush-50 space-y-5">
            ...
            <div>
              ...
              <input type="text" {...register('name',{required:'Name is required'})}
                className="w-full px-4 py-3 border border-blush-200 rounded-xl text-sm text-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-400/50 focus:border-brand-400 transition-all"/>
            ...
            <div>
              ...
              <textarea rows={3} {...register('description',{required:'Description is required'})}
                className="w-full px-4 py-3 border border-blush-200 rounded-xl text-sm text-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-400/50 focus:border-brand-400 transition-all resize-none"/>
